---
description: Save project discoveries and decisions to Neurox persistent memory
agent: planner
subtask: true
---

Save important project discoveries and decisions to Neurox persistent memory for future sessions.
...
5. For each discovery, save to Neurox with:
   - A searchable title
...
These observations will be available in future sessions via `neurox_context` or `neurox_recall`.
```
...
- As the first memory action, call `neurox_session_start`, then `neurox_context` before any other context retrieval
- Use `neurox_recall` first to avoid duplicating existing observations
